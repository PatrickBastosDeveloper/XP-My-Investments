...
const isValideLogin = rescue((req, _res, next) => {
  ...
  if (!email) {
    throw new HttpException(401, 'Invalid email address.');
  }
  if (!password) {
    throw new HttpException(401, 'Invalid password.');
  }
  if (!emailVerification(email)) {
    throw new HttpException(401, 'Invalid email format.');
  }
  if (password.length < MIN_CHARACTERS) {
    throw new HttpException(401, 'Your password need be at least 8 characters long.');
  }
